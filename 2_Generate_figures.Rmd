...
if (!file.exists(file.path(figFolder, "new_HIV_infection1.png"))) {
    SaveFigure(figFolder,"new_HIV_infection1",newinfectionfig)
}
...
#                     labels = c(projectSpecs$startyear,
#                     as.character(1:(projectSpecs$steps-1))))+
  ylim(signif(max(cumulatedinfectionresults$cumulatedHIV),digits = 1),signif(max(cumulatedinfectionresults$cumulatedHIV),digits = 2))
cumulatedinfectionfig

# If figure not already created make sure it is not overwritten
if (!file.exists(file.path(figFolder, "Cumulated_HIV_infection1.png"))) {
    SaveFigure(figFolder,"Cumulated_HIV_infection1",cumulatedinfectionfig)
}
